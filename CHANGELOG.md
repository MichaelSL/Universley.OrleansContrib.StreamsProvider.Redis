...
- **Failed publishes throw.** `OnNextAsync` now fails when Redis rejects a write, instead of logging and dropping the event. If one call publishes several events and a later one fails, the earlier ones are already in the stream, so a retry can duplicate them.
